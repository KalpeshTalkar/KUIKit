...
public class KTextField: UITextField {
    ...
    /// Sets the underline color
    @IBInspectable public var underLineColorWhenFocused: UIColor? = nil {
        didSet {
            updateUnderLineUI()
        }
    }
    ...
    /// Sets left margin
    @IBInspectable public var leftMargin: CGFloat = 5.0 {
        didSet {
            setMargins()
        }
    }
    ...
    /// Sets right margin
    @IBInspectable public var rightMargin: CGFloat = 5.0 {
        didSet {
            setMargins()
        }
    }
    ...
    private func applyStyles() {
        applyUnderLine()
        setMargins()
        addTarget(self, action: #selector(editingChanged), for: UIControlEvents.editingDidBegin)
        addTarget(self, action: #selector(editingChanged), for: UIControlEvents.editingDidEnd)
    }
    ...
    private func updateUnderLineUI() {
        if isEditing {
            underLineLayer.backgroundColor = underLineColorWhenFocused != nil ? underLineColorWhenFocused!.cgColor : tintColor.cgColor
        } else {
